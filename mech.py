...
def redrawgamehero():
    ...
    if walkcount+1>=12:
        walkcount=0
    if right:
        screen.blit(walkRight[walkcount//3],hero_size)
        walkcount+=1
    elif left:
        screen.blit(walkLeft[walkcount // 3], hero_size)
        walkcount += 1
    elif up and not(right) and not(left):
        screen.blit(walkUP[walkcount // 3], hero_size)
        walkcount += 1
    elif down and not(right) and not(left):
        screen.blit(walkDOWN[walkcount // 3], hero_size)
        walkcount += 1
    else:
        screen.blit(stand,hero_size)
